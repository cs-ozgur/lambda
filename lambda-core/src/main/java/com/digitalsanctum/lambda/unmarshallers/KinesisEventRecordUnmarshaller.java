...
import com.amazonaws.services.kinesis.model.transform.RecordJsonUnmarshaller;
...
public class KinesisEventRecordUnmarshaller implements Unmarshaller<KinesisEvent.KinesisEventRecord, JsonUnmarshallerContext> {
  ...
  public KinesisEventRecordUnmarshaller() {
  }
      ...
      for (; token != null; token = context.nextToken()) {
        if (token != JsonToken.FIELD_NAME && token != JsonToken.START_OBJECT) {
          ...
          if (context.testExpression("kinesis", targetDepth)) {
            context.nextToken();
            kinesisEventRecord.setKinesis((KinesisEvent.Record) RecordJsonUnmarshaller.getInstance().unmarshall(context));
          }
          //eventSourceARN
